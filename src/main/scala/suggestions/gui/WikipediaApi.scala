...
trait WikipediaApi {
  ...
  /** Returns an `Observable` with the contents of the Wikipedia page for a given search `term`.
    */
  ...
  implicit class ObservableOps[T](obs: Observable[T]) {
    ...
    def recovered: Observable[Try[T]] = {
      obs.map { s => Success(s) }.onErrorReturn { e => Failure(e) }
    }
    ...
    /** Emits the events from the `obs` observable, until `totalSec` seconds have elapsed.
      ...
      */
    def timedOut(totalSec: Long): Observable[T] = obs.window(totalSec seconds).first.flatten
